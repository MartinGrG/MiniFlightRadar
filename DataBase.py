...
class infosVols():
    def __init__(self, aeroport, debut, fin):
        ...
        :param fin:
        """
        #Création du dataFrame contenant les informations des vols retenus
        self.vols = self.easa(self.FAA(self.compagnie(self.vol_aeroport(aeroport, debut, fin))))  # Ajout de la colonne index
        self.vols.insert(0, 'index', range(1, len(self.vols) + 1))
        self.vols.to_csv('flights_data.csv', index=False)  # Sauvegarde le DataFrame dans un fichier CSV

        self.trajectoire = None  # Trajectoire de l'avion sous forme de liste, mise à jour avec la méthode airplane_traj
        self.donneesMoteur = None   # Donnees du moteur, mise à jour avec la méthode emission

    def vol_aeroport(self, aeroport, debut, fin):
        """
    ...
    def FAA(self, DF):
        """
        Fonction comparant et croisant le DataFrame en entrée avec la base de donnée de la FAA.
    ...
    def airplane_traj(self, index):
        ...
        self.trajectoire = data.path


    def compagnie(self, DF):
        """
    ...
    def emission(self, index):
        ...
        self.donneesMoteur = ligne


    def sortie(self, aeroport, debut, fin):
        """
        ...
        """
        DF = self.easa(self.FAA(self.compagnie(self.vol_aeroport(aeroport, debut, fin))))  # Ajout de la colonne index
